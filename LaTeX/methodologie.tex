...
Ansible biedt met Ansible Tower een ge\"integreerde monitoringstool. Bovendien wordt Ansible door velen geprezen vanwege zijn eenvoud in syntax en configuratie. Zo heeft Ansible geen agent nodig om servers te configureren wat een enorm voordeel biedt. Er kan dus zonder iets te moeten installeren onmiddellijk overgegaan worden tot configureren. Ansible Tower kan met vele externe technologie\"en overweg wat de mogelijkheden tot continuous deployment vereenvoudigt. Zo is bijvoorbeeld git ingebouwd wat zorgt voor een eenvoudig beheer van de source code.
...
Servers kunnen eenvoudig corrupt geraken. Malafide personen kunnen binnendringen en bestanden aanpassen zoals bijvoorbeeld een firwall uitzetten. Weliswaar hoeft het niet altijd zo drastisch te lopen. Vaak worden servers beschadigd door intern personeel die goedbedoelt updates wilt doorvoeren maar met onvoorziene nadelige consequenties tot gevolg.
Een \gls{CMT} kan hier een oplossing bieden.  Aangezien een goed geconfigureerde \gls{CMT} op regelmatige basis de nodige servers controlleerd kunnen snel mogelijke gevaren vastgesteld worden en opgelost. Stel dat bijvoorbeeld een verkeerde gebruikersgroep toegang heeft gekregen tot een belangrijke productieserver door een menselijke fout. Wanneer twee uur later de \gls{CMT} de server zijn configuratie controlleerd zal deze vaststellen dat de gebruikersgroepen niet meer in orde zijn en zal deze aanpassen \footnote{Het kan weliswaar gebeuren dat deze aanpassingen terecht zijn (en dat de configuratie van de \gls{CMT} verouderd zijn. Daarom zal de VRT ansible playbooks laten draaien in check-modus. Hiebij controlleerd hij de server maar voert geen aanpassingen door.}. Normaal komen veiligheidsproblemen pas heel laat aan het licht en meestal zelfs als het al te laat is. Dankzij deze \gls{CMT} kan dit drastisch gerduseert worden. Belangrijk bovendien is dat de rapporten die de \gls{CMT} genereerd op regelmatige basis gecontrolleerd worden.  Wanneer een belangrijke configuratie om een verkeerde mannier aangepast is geweest moet misschien de oorzaak hiervan onderzocht worden om dit in de toekomst te kunnen voorkomen.

Een goede beveiliging van de Puppetmaster of Ansible Tower is van allergrootst belang. Dit omdat deze server toegang heeft tot verschillende servers. Wanneer een specifieke server in verkeerde handen valt kan dit vanzelfsprekend tot verregaande gevolgen leiden maar de impact op andere servers blijft beperkt. Wanneer dit de Ansible- of Puppetmaster betreft is de impact veel groter aangezien deze vele servers configureerd. Deze master heeft niet enkel toegang tot vele andere servers maar doorgaans ook verregaande rechten om nodige zaken te kunnen configureren.
Bij grotere bedrijven worden Ansible rollen en Puppet modules doorgaans bewaart op daarvoor bestemde servers bedoelt voor source control. Ook voor deze server in het van belang dat dat enkel speciefiek personen toegang krijgen tot specifieke rollen en modules. Zo moet de database admin enkel toegang krijgen tot code betreffende databanken terwijl dat voor andere personen weer andere rollen en modules zullen zijn.

Zowel Ansible als Puppet schenken de nodige aandacht om zo veilig mogelijk te werk te gaan. Beiden gebruiken encryptie in hun vorm van communiceren. Puppet maakt hierbij gebruik van HTTPS terwijl dit bij Ansible via SSH wordt geregeld.
Bij Puppet moet elke server handmatig geautoriseerd worden alvorens deze configuraties bij de master kunnen aanvragen. Bij Ansible is dit niet nodig. Hij bepaald zelf welke servers hij wenst te configureren. Weliswaar dient hij toegang te krijgen tot elk van deze servers.

Het feit dat Ansible veiliger is dan Puppet is moeilijk te bewijzen. Er zijn anderzijds enkele duidelijke aanwijzingen waarbij Ansible een voordeel heeft op gebied van veiligheid. Bij Puppet dient poort 8140 voordurend open te staan om aanvragen te behandelen. Bij Ansible worden deze configuraties gestart door de master zelf. Hierbij moet wel poort 22 (SSH) open staan voor op elke client. Maar dit is doorgaans sowieso al van toepassingen. Bovendien heeft Ansible geen deamon zoals Puppet die op rootlevel geïnstalleerd is geweest.
...
\subsection{Imlementatie van Ansible}
Ansible is relatief eenvoudig in implementatie. In de omgeving van de VRT maakt Puppet gebruik van \gls{LDAP}. Hieruit kan hij de nodige variabelen ophalen. Vervolgens configureerde Puppet de servers met deze waarden. Ideaal is de nieuwe Ansible integratie zo analoog mogelijk aan de huidige mannier van werking. Om deze waarden op te kunnen halen uit \gls{LDAP} voorziet Ansible Tower geen standaard functionaliteit. Dit moet opgevangen worden door gebruik te maken van een script. Hierbij dient Ansible twee methodes op te kunnen roepen. Een eerste voor het populeren van groepen en hosts en een tweede voor het toevoegen van variabelen voor elk van deze hosts. Ansible Tower verwacht dat deze methodes JSON retourneren. 

In tweede instantie dient de source control server correct ingesteld te staan. Niet alleen op gebied van rechten zoals aagehaald is geweest in \ref{sec:veiligheid} maar ook op gebied van configuratie. Aangezien servers in productie niet dezelfde configuratie mogen hebben als servers in staging moet een duidelijk onderscheid gemaakt worden. Dit kan verwezelijkt worden door gebruik te maken van git branches. Vervolgens wordt elke branch gekoppeld met de corresponderende groep servers die worden gegenereerd zoals beschreven in voorgaande sectie.
Om hierbij aan een goed versiebeheer te kunnen doen wordt elke Ansible rol in git toegevoegd als een submodule. Elke commit gaat gepaart met een versienummer. Dit stelt de gebruikers in staat om eenvoudig een roll-back te doen in het geval dat bepaalde updates zouden mislukken.

Eens de inventories aangemaakt worden en de source code opgehaald is geweest is het aan Ansible Tower de taak om deze twee te combineren en uit te voeren op de servers. Hiervoor dient hij uiteraard toegang te krijgen op de desbetreffende servers.
...
Ansible berekend de prijs op een lichtjes andere manier dan Puppet. Bijgevolg hangt de prijs af van de situatie. Voor standaard enterprise support rekent Ansible 9.092,15 euro voor 100 nodes. Puppet berekend de prijst op het aantal gebruikte nodes. Dit is dan 109,12 euro per node. Bij 100 nodes komt Puppet dus duurder uit. Heb je echter minder als 100 nodes is Ansible duurder want je betaald standaard voor 100 nodes. Bij meerdere nodes dient een offerte aangevraagd te worden. Ook voor de premium editie dient voor beide technologie\"en een offerte aangevraagd te worden. \footnote{De prijzen zijn geraadpleegd geweest op mei 2017 en zijn omgerekend van USD naar EUR tegen de wisselkoers op 5/5/2017}
